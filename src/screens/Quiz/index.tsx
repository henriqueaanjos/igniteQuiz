...
export function Quiz() {
  ...
  const shakeAnimationStyle = useAnimatedStyle(() => ({
    transform: [{ 
      translateX: interpolate(shake.value, [0, 0.5, 1, 1.5, 2, 2.5, 3], [0, -15, 0, 15, 0, -15, 0])
    }]
  }));
  ...
  const animatedFixedHeaderStyle = useAnimatedStyle(() => {
    return{
      width: '110%',
      left: '-5%',
      position: 'absolute',
      ...
      opacity: interpolate(scroll.value, [50, 90], [0, 1], Extrapolate.CLAMP),
      transform: [{
        translateY: interpolate(scroll.value, [50, 100], [-40, -0], Extrapolate.CLAMP)
      }]
    }
  });
  ...
  const animateHeaderStyle = useAnimatedStyle(() => {
    return{
      opacity: interpolate(scroll.value, [60, 90], [1, 0], Extrapolate.CLAMP)
    }
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View
        style={animatedFixedHeaderStyle}
      >
      ...
      >
      <Animated.View style={[animateHeaderStyle, styles.header]}>
        <QuizHeader
          title={quiz.title}
